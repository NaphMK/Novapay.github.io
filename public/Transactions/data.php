  ...
  if ($wallet_balance >= $price) {
      ...
      if ($status_resp === "successful") {
          ...
          if ($stmt3->affected_rows >= 0) {
              ...
              // Update the data_trans table with the API response
              $stmt4 = $mysqli->prepare("UPDATE data_trans SET api_response = $result  WHERE id = ?");
              $stmt4->bind_param("si", $result, $transaction_id);
              $stmt4->execute();
              $stmt4->close();
